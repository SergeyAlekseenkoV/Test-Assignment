<?php include_once "header.php"; ?>
<title>5С-Виробництво</title>
</head>

<body id="main">
    <?php include_once "menu.php"; ?>
    <!-- main-section-->
    <section class="mt-0 pt-3 pb-lg-5 best_team">
        <div class="container pt-2 pb-2 pb-lg-3 px-sm-0 px-md-3 px-lg-3 px-xl-3">
            <div class="d-flex justify-content-between align-items-baseline mt-3 mt-md-3 mt-lg-3 mt-xl-4">
                <h3 class="d-md-none d-lg-block pt-sm-2 pt-lg-3 text-left text-uppercase text_color wow slideInLeft"
                    data-animation="slideInLeft">5С-Виробництво</h3>
                <h3
                    class="d-none d-md-block align-self-baseline d-lg-none pt-sm-2 pt-lg-3 text-left mb-2 text-uppercase text_color">
                    Акціонерне Товариство "Марганецький гірничо-збагачувальний
                    комбінат"</h3>
                <a href="index.php"
                    class="align-self-baseline d-flex flex-row align-items-center flex-nowrap bg-light mt-5 mb-3 p-1 text-uppercase border border-primary rounded shadow text_color">
                    Вихід<i class="fas fa-sign-out-alt ml-2"></i></a>
            </div>
            <div class="row pb-sm-1 mx-0 px-0">
                <?php include_once "left-nav.php"; ?>
                <!-- aside -->
                <aside class="col-sm-12 col-lg-10 px-0 px-sm-0 pl-md-0 pl-lg-1 py-3 py-lg-0 wow fadeInUp enterprise-5s"
                    data-animation="fadeInUp">
                    <div class="row mx-0 p-1 border border-primary rounded">
                        <div class="col-12 mx-0 px-0 text-center mb-1 mb-md-0">
                            <h2 class="text-uppercase px-0 py-3 mx-0 my-0 mb-md-1 text-white bg_headers">Каталог кращих
                                прикладів</h2>
                        </div>
                        <div class="col-12 m-0 px-0 border border-primary position-relative overflow-hidden">
                            <img src="../build/img/bg-enterprise.jpg" alt="photo" class="w-100 obj-fit-center height-100p">
                            <div
                                class="d-flex flex-column justify-content-center align-items-center position-absolute img_button_block bg_dark_through">
                                <button id="show-btn"
                                    class="w-100 min-height btn-transparent position-absolute">
                                    <span data-toggle="tooltip" data-placement="bottom" data-original-title="Каталог кращих прикладів">&#10095;</span></button>
                            </div>
                            <div
                                class="d-flex flex-column justify-content-center align-items-center title position-absolute bg_info_through">
                                <span class="text-uppercase"><strong class="shadows">5С-Виробництво</strong></span>
                            </div>
                        </div>
                        <div id="block-examples" class="col-12 px-0">
                            <div class="row w-100 mx-0 mt-1 mb-0 bg_info_through">
                                <div class="col-12 px-0">
                                    <h4 class="text-center">Шахта 9-10</h4>
                                    <div class="row w-100 mx-0">
                                        <div class="col-6 p-0 thumb mx-auto overflow-hidden height-210">
                                            <a data-fancybox="gallery" href="../build/img/media/image2.jpg" class="position-relative little-filter-effect d-block height-100p text_color">
                                            <p class="lead position-absolute mt-1 ml-1 mb-0 px-2 bg_info_through">АТЦ</p>
                                                <img src="../build/img/media/image2.jpg" alt="photo"
                                                    class="img-fluid p-1 obj-fit w-100 height-100p">
                                            </a>
                                        </div>
                                        <div class="col-6 p-0 thumb mx-auto overflow-hidden height-210">
                                            <a data-fancybox="gallery" href="../build/img/media/image3.jpg" class="position-relative little-filter-effect d-block height-100p text_color">
                                                <p class="lead position-absolute mt-1 ml-1 mb-0 px-2 bg_info_through">АТЦ</p>
                                                <img src="../build/img/media/image3.jpg" alt="photo"
                                                    class="img-fluid p-1 obj-fit w-100 height-100p">
                                            </a>
                                        </div>
                                    </div>
                                </div>
                                <div class="col-12 px-0 text-center">
                                    <h4 class="text-center">Маркування зон зберігання</h4>
                                    <div class="row w-100 mx-0">
                                        <div class="col-6 p-0 thumb mx-auto overflow-hidden height-210">
                                            <a data-fancybox="gallery" href="../build/img/media/image4.jpg" class="position-relative little-filter-effect d-block height-100p text_color">
                                                <p class="lead position-absolute mt-1 ml-1 mb-0 px-2 bg_info_through">АТЦ</p>
                                                <img src="../build/img/media/image4.jpg" alt="photo"
                                                    class="img-fluid p-1 obj-fit w-100 height-100p">
                                            </a>
                                        </div>
                                        <div class="col-6 p-0 thumb mx-auto overflow-hidden height-210">
                                            <a data-fancybox="gallery" href="../build/img/media/image5.jpeg" class="position-relative little-filter-effect d-block height-100p text_color">
                                                <p class="lead position-absolute mt-1 ml-1 mb-0 px-2 bg_info_through">Госп. цех</p>
                                                <img src="../build/img/media/image5.jpeg" alt="photo"
                                                    class="img-fluid p-1 obj-fit w-100 height-100p">
                                            </a>
                                        </div>
                                    </div>
                                </div>
                                <div class="col-12 px-0 text-center">
                                    <h4 class="text-center">Маркування зон зберігання велико&shy;габаритного пересувного обладнання</h4>
                                    <div class="row w-100 mx-0">
                                        <div class="col-6 p-0 thumb mx-auto overflow-hidden height-210">
                                            <a data-fancybox="gallery" href="../build/img/media/image6.jpg" class="position-relative little-filter-effect d-block height-100p text_color">
                                                <p class="lead position-absolute mt-1 ml-1 mb-0 px-2 bg_info_through">СЛЗ</p>
                                                <img src="../build/img/media/image6.jpg" alt="photo"
                                                    class="img-fluid p-1 obj-fit w-100 height-100p">
                                            </a>
                                        </div>
                                        <div class="col-6 p-0 thumb mx-auto overflow-hidden height-210">
                                            <a data-fancybox="gallery" href="../build/img/media/image7.jpg" class="position-relative little-filter-effect d-block height-100p text_color">
                                                <p class="lead position-absolute mt-1 ml-1 mb-0 px-2 bg_info_through">РБЦ</p>
                                                <img src="../build/img/media/image7.jpg" alt="photo"
                                                    class="img-fluid p-1 obj-fit w-100 height-100p">
                                            </a>
                                        </div>
                                    </div>
                                </div>
                                <div class="col-12 px-0 text-center">
                                    <h4 class="text-center">Маркування робочої зони обладнання в ЦМП</h4>
                                    <div class="row w-100 mx-0">
                                        <div class="col-6 p-0 thumb mx-auto overflow-hidden height-210">
                                            <a data-fancybox="gallery" href="../build/img/media/image8.jpg" class="position-relative little-filter-effect d-block height-100p text_color">
                                                <p class="lead position-absolute mt-1 ml-1 mb-0 px-2 bg_info_through"></p>
                                                <img src="../build/img/media/image8.jpg" alt="photo"
                                                    class="img-fluid p-1 obj-fit w-100 height-100p">
                                            </a>
                                        </div>
                                        <div class="col-6 p-0 thumb mx-auto overflow-hidden height-210">
                                            <a data-fancybox="gallery" href="../build/img/media/image9.jpg" class="position-relative little-filter-effect d-block height-100p text_color">
                                                <p class="lead position-absolute mt-1 ml-1 mb-0 px-2 bg_info_through"></p>
                                                <img src="../build/img/media/image9.jpg" alt="photo"
                                                    class="img-fluid p-1 obj-fit w-100 height-100p">
                                            </a>
                                        </div>
                                    </div>
                                </div>
                                <div class="col-12 px-0 text-center">
                                    <h4 class="text-center">Центральна енерго-механічна лабораторія</h4>
                                    <div class="row w-100 mx-0">
                                        <div class="col-6 p-0 thumb mx-auto overflow-hidden height-210">
                                            <a data-fancybox="gallery" href="../build/img/media/image10.jpg" class="position-relative little-filter-effect d-block height-100p text_color">
                                                <p class="lead position-absolute mt-1 ml-1 mb-0 px-2 bg_info_through"></p>
                                                <img src="../build/img/media/image10.jpg" alt="photo"
                                                    class="img-fluid p-1 obj-fit w-100 height-100p">
                                            </a>
                                        </div>
                                        <div class="col-6 p-0 thumb mx-auto overflow-hidden height-210">
                                            <a data-fancybox="gallery" href="../build/img/media/image11.jpg" class="position-relative little-filter-effect d-block height-100p text_color">
                                                <p class="lead position-absolute mt-1 ml-1 mb-0 px-2 bg_info_through"></p>
                                                <img src="../build/img/media/image11.jpg" alt="photo"
                                                    class="img-fluid p-1 obj-fit w-100 height-100p">
                                            </a>
                                        </div>
                                    </div>
                                </div>
                                <div class="col-12 px-0 text-center">
                                    <h4 class="text-center">РМЦ</h4>
                                    <div class="row w-100 mx-0">
                                        <div class="col-6 p-0 thumb mx-auto overflow-hidden height-210">
                                            <a data-fancybox="gallery" href="../build/img/media/image12.jpg" class="position-relative little-filter-effect d-block height-100p text_color">
                                                <p class="lead position-absolute mt-1 ml-1 mb-0 px-2 bg_info_through"></p>
                                                <img src="../build/img/media/image12.jpg" alt="photo"
                                                    class="img-fluid p-1 w-100 obj-fit height-100p">
                                            </a>
                                        </div>
                                        <div class="col-6 p-0 thumb mx-auto overflow-hidden height-210">
                                            <a data-fancybox="gallery" href="../build/img/media/image13.jpg" class="position-relative little-filter-effect d-block height-100p text_color">
                                                <p class="lead position-absolute mt-1 ml-1 mb-0 px-2 bg_info_through"></p>
                                                <img src="../build/img/media/image13.jpg" alt="photo"
                                                    class="img-fluid p-1 w-100 obj-fit height-100p">
                                            </a>
                                        </div>
                                        <div class="col-6 p-0 thumb mx-auto overflow-hidden height-210">
                                            <a data-fancybox="gallery" href="../build/img/media/image14.jpg" class="position-relative little-filter-effect d-block height-100p text_color">
                                                <p class="lead position-absolute mt-1 ml-1 mb-0 px-2 bg_info_through"></p>
                                                <img src="../build/img/media/image14.jpg" alt="photo"
                                                    class="img-fluid p-1 obj-fit w-100 height-100p">
                                            </a>
                                        </div>
                                    </div>
                                </div>
                                <div class="col-12 px-0 text-center">
                                    <h4 class="text-center">АТЦ</h4>
                                    <div class="row w-100 mx-0">
                                        <div class="col-6 p-0 thumb mx-auto overflow-hidden height-210">
                                            <a data-fancybox="gallery" href="../build/img/media/image15.jpg" class="position-relative little-filter-effect d-block height-100p text_color">
                                                <p class="lead position-absolute mt-1 ml-1 mb-0 px-2 bg_info_through"></p>
                                                <img src="../build/img/media/image15.jpg" alt="photo"
                                                    class="img-fluid p-1 obj-fit w-100 height-100p">
                                            </a>
                                        </div>
                                        <div class="col-6 p-0 thumb mx-auto overflow-hidden height-210">
                                            <a data-fancybox="gallery" href="../build/img/media/image16.jpg" class="position-relative little-filter-effect d-block height-100p text_color">
                                                <p class="lead position-absolute mt-1 ml-1 mb-0 px-2 bg_info_through"></p>
                                                <img src="../build/img/media/image16.jpg" alt="photo"
                                                    class="img-fluid p-1 obj-fit w-100 height-100p">
                                            </a>
                                        </div>
                                    </div>
                                </div>
                                <div class="col-12 px-0 text-center">
                                    <h4 class="mx-auto">Зберігання інструментів</h4>
                                    <div class="row w-100 mx-0">
                                        <div class="col-6 p-0 thumb mx-auto overflow-hidden height-210">
                                            <a data-fancybox="gallery" href="../build/img/media/image17.jpeg" class="position-relative little-filter-effect d-block height-100p text_color">
                                                <p class="lead position-absolute mt-1 ml-1 mb-0 px-2 bg_info_through">АТЦ</p>
                                                <img src="../build/img/media/image17.jpeg" alt="photo"
                                                    class="img-fluid p-1 obj-fit w-100 height-100p">
                                            </a>
                                        </div>
                                        <div class="col-6 p-0 thumb mx-auto overflow-hidden height-210">
                                            <a data-fancybox="gallery" href="../build/img/media/image18.jpeg" class="position-relative little-filter-effect d-block height-100p text_color">
                                                <p class="lead position-absolute mt-1 ml-1 mb-0 px-2 bg_info_through">ЦТДЗ</p>
                                                <img src="../build/img/media/image18.jpeg" alt="photo"
                                                    class="img-fluid p-1 obj-fit w-100 height-100p">
                                            </a>
                                        </div>
                                        <div class="col-6 p-0 thumb mx-auto overflow-hidden height-210">
                                            <a data-fancybox="gallery" href="../build/img/media/image19.jpg" class="position-relative little-filter-effect d-block height-100p text_color">
                                                <p class="lead position-absolute mt-1 ml-1 mb-0 px-2 bg_info_through">АТЦ</p>
                                                <img src="../build/img/media/image19.jpg" alt="photo"
                                                    class="img-fluid p-1 obj-fit w-100 height-100p">
                                            </a>
                                        </div>
                                    </div>
                                </div>
                                <div class="col-12 px-0 text-center border-top">
                                    <h4 class="mx-auto"></h4>
                                    <p class="lead pb-0">
                                        <strong>КАТАЛОГ кращих прикладів 5С</strong> - виробництво буде поповнюватися постійно.
                                        Чекаємо ваших фотографій на електронну адресу <address><strong>boumgok@gmail.com</strong></address>
                                    </p>
                                </div>
                            </div>
                        </div>
                    </div>
                </aside>
            </div>
        </div>
    </section>
    <!--footer-->
    <script>
        $(document).ready(function () {
            $("#show-btn").click(function () {
                $("#block-examples").slideToggle();
            });
        });
    </script>
    <?php include_once "footer.php"; ?>